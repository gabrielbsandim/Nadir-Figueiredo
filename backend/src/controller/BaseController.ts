...
export abstract class BaseController<T> extends BaseNotification {
    ...
    async all(request: Request) {
        if (this.checkPermission(request)) return this.errorRoot;
    ...
    async save(model: any, request: Request) {
        ...
        if (model.uid) {
            ...
            const _modelInDB = await this._repository.findOne(uid);
            if (_modelInDB) {
    ...
    async remove(request: Request) {
        ...
        const model: any = await this._repository.findOne(uid);
        if (model) {
            model.deleted = true;
